...
class GameManager extends Component {
  ...
  startTime = () => {
    this.intervalId = setInterval(this.timeDrop, 1000)
  }
  ...
  onActiveTabThumbnail = () => {
    const {activeTabs} = this.state
    return imagesList.filter(eachImage => eachImage.category === activeTabs)
  }

  onImageChange = () => {
    const randomImage = Math.floor(Math.random() * imagesList.length)
    return imagesList[randomImage]
  }

  onThumbnailClick = id => {
    const {matchImgGame} = this.state
    if (matchImgGame.id === id) {
      this.setState(oldValue => ({
        score: oldValue.score + 1,
        matchImgGame: this.onImageChange(),
      }))
    } else {
  ...
  render() {
    const {score, isPlay, timer, activeTabs, matchImgGame} = this.state
    const filterThumbnail = this.onActiveTabThumbnail()
    return (
          ...
          {isPlay ? (
            <div className="play-card">
              <div className="image-container">
                <img
                  src={matchImgGame.imageUrl}
                  alt="match"
                  className="fruit-img"
                />
              </div>
              <ul className="each-tab-container">
                {tabsList.map(eachTab => (
                  <TabList
                    key={eachTab.tabId}
                    tabDetail={eachTab}
                    onActiveTab={this.onActiveTab}
                    isActive={activeTabs === eachTab.tabId}
                  />
                ))}
              </ul>

              <ul className="each-img-container">
                {filterThumbnail.map(eachImg => (
                  <ThumbnailList
                    key={eachImg.id}
                    thumbnailDetail={eachImg}
            ...
            <div className="result-card">
              ...
              >
                <img
                  ...
                  alt="reset"
                />
                <p className="reset-card">Play Again</p>
              </button>
            </div>
          )}
        </div>
